...
const MatriculeAutorise = require('../models/MatriculeAutorise'); // ← cette ligne manque
...
    if (role === 'enseignant') {
    const matriculeDoc = await MatriculeAutorise.findOne({ matricule });

    if (!matriculeDoc) {
      return res.status(400).json({ message: 'Matricule non reconnu' });
    }
    if (matriculeDoc.utilise) {
      return res.status(400).json({ message: 'Matricule déjà utilisé' });
    }

    // Marquer le matricule comme utilisé
    matriculeDoc.utilise = true;
    await matriculeDoc.save();
  }
